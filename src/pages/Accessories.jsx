...
function Accessories() {
  ...
  const list = useMemo(() => {
    let filtered = products.filter((item) => item.category === 'Accessory')
    if (inStockOnly) filtered = filtered.filter((item) => item.stock > 0)
    if (minPrice) filtered = filtered.filter((item) => item.price >= Number(minPrice))
    if (maxPrice) filtered = filtered.filter((item) => item.price <= Number(maxPrice))
    switch (sortBy) {
      case 'price-asc':
    ...
    return filtered
  }, [inStockOnly, maxPrice, minPrice, products, sortBy])
  ...
  return (
    <div className="page">
      <section className="page-hero">
        <div>
          <h1>Accessories</h1>
          <p>Upgrade your setup with premium accessories and desk gear.</p>
        </div>
